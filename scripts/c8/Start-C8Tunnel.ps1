...
$admission = & $python -m systeme_local_gateway.c8_live_cycle verify-bundle `
    --bundle (Join-Path $state "live-cycle.json")
if ($LASTEXITCODE -ne 0) {
    throw "C8 Tunnel startup refused an invalid or stale live-cycle grant."
}
$facadePid = Read-C8Pid -Name "facade"
if ($null -eq $facadePid) {
    throw "Start the C8 facade before the Tunnel."
}
Assert-C8LoopbackListener -ProcessId $facadePid -Port 8765
if ($null -ne (Read-C8Pid -Name "tunnel")) {
    throw "C8 tunnel-client PID file already exists."
}
$tunnel = Assert-C8TunnelBinary
...
try {
    ...
    foreach ($attempt in 1..20) {
        if ($process.HasExited) {
            throw "Secure MCP Tunnel client exited during C8 startup."
        }
        try {
    ...
    if (-not $ready) {
        throw "Secure MCP Tunnel C8 readiness timed out."
    }
    Assert-C8LoopbackListener -ProcessId $process.Id -Port 8766
} catch {
    Stop-C8Process -Name "tunnel" -AllowedExecutableNames @("tunnel-client.exe")
    throw
}
