...
| `C1-Q20` | `2026-07-27T01:06Z` to `01:08Z` | First GitHub Actions run for draft PR `#67` | MIXED: the main Python job and Windows Rust job passed; Python 3.14 ran 872 tests successfully before the change-seal test failed because its shallow checkout did not contain exact C0 base commit `912d0d33e119469ff957965104cf20af5e491923`; Rust quality was still running when diagnosed. The focused CI correction adds `fetch-depth: 0` only to the Python 3.14 checkout, matching the already successful main Python job. |
| `C1-Q21` | `2026-07-27T01:16Z` | Local validation of the Python 3.14 checkout correction and future-provider documentation | PASS: workflow YAML parsed; the compatibility job's checkout contract requires `fetch-depth: 0`; 86 targeted C1 tests passed; links remained valid across 38 Markdown files; deterministic 40-file seal verification passed; `git diff --check` was clean; bounded credential scan found zero values. |
| `C1-Q22` | `2026-07-27T01:18Z` to `01:21Z` | Second GitHub Actions run for draft PR `#67` after the checkout correction | PASS: main Python job in 56 seconds; Python 3.14 compatibility in 32 seconds; Windows Rust in 1 minute 5 seconds; Rust quality and dependency audit in 2 minutes 58 seconds. All four required jobs completed successfully. |
